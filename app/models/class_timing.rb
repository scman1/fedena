...
class ClassTiming < ActiveRecord::Base
  ...
  scope :for_batch, lambda { |b| { :conditions => { :batch_id => b.to_i, :is_deleted=>false, :is_break => false},  :order =>'start_time ASC' } }
  scope :default, :conditions => { :batch_id => nil, :is_break => false, :is_deleted=>false  }, :order =>'start_time ASC'
  scope :active_for_batch, lambda { |b| { :conditions => { :batch_id => b.to_i, :is_deleted=>false},  :order =>'start_time ASC' } }
  scope :active, :conditions => { :batch_id => nil, :is_deleted=>false  }, :order =>'start_time ASC'
  ...
  def validate
    ...
    errors.add_to_base( "#{I18n.t('class_time_overlaps_with_existing')}.") if between_overlap
