...
class EmployeeController < ApplicationController
  ...
  def edit_category
    @category = HR::EmployeeCategory.find(params[:id])
      ...
      if (params[:category][:status] == 'false' and employees.blank?) or params[:category][:status] == 'true'
        ...
        if @category.update_attributes(params[:category])
          unless @category.status
            position = HR::EmployeePosition.find_all_by_employee_category_id(@category.id)
            position.each do |p|
  ...
  def admission1
    ...
    @employee = HR::Employee.new(params[:employee])
    @selected_value = CustomSetting.default_country
    @last_admitted_employee = HR::Employee.find(:last,:conditions=>"employee_number != 'admin'")
    ...
    if request.post?
      ...
      unless @employee.employee_number.to_s.downcase == 'admin'
        if @employee.save
          @leave_type = HR::EmployeeLeaveType.all
          @leave_type.each do |e|
            HR::EmployeeLeave.create( :employee_id => @employee.id, :employee_leave_type_id => e.id, :leave_count => e.max_leave_count)
          end
  ...
  def edit3
    ...
    if request.post?
      params[:employee_bank_details].each_pair do |k, v|
        row_id= HR::EmployeeBankDetail.find_by_employee_id_and_bank_field_id(@employee.id,k)
        unless row_id.nil?
          bank_detail = HR::EmployeeBankDetail.find_by_employee_id_and_bank_field_id(@employee.id,k)
          HR::EmployeeBankDetail.update(bank_detail.id,:bank_info => v['bank_info'])
        else
          HR::EmployeeBankDetail.create(:employee_id=>@employee.id,:bank_field_id=>k,:bank_info=>v['bank_info'])
        end
      end
  ...
  #        HR::EmployeeAdditionalDetail.create(:employee_id => params[:id],
  ...
  def admission3_1
    @employee = HR::Employee.find(params[:id])
    @employee_additional_details = HR::EmployeeAdditionalDetail.find_all_by_employee_id(@employee.id)
    ...
    if request.post?
      ...
      unless @error==true
        params[:employee_additional_details].each_pair do |k, v|
          ...
          prev_record = HR::EmployeeAdditionalDetail.find_by_employee_id_and_additional_field_id(params[:id], k)
          unless prev_record.nil?
            prev_record.update_attributes(:additional_info => addl_info)
          else
            addl_detail = HR::EmployeeAdditionalDetail.new(:employee_id => params[:id],
  ...
  def edit3_1
    ...
    if request.post?
      params[:employee_additional_details].each_pair do |k, v|
        row_id= HR::EmployeeAdditionalDetail.find_by_employee_id_and_additional_field_id(@employee.id,k)
        unless row_id.nil?
          additional_detail = HR::EmployeeAdditionalDetail.find_by_employee_id_and_additional_field_id(@employee.id,k)
          HR::EmployeeAdditionalDetail.update(additional_detail.id,:additional_info => v['additional_info'])
        else
          HR::EmployeeAdditionalDetail.create(:employee_id=>@employee.id,:additional_field_id=>k,:additional_info=>v['additional_info'])
        end
      end
      flash[:notice] = "#{t('flash15')}#{@employee.first_name} #{t('flash14')}"
      redirect_to :action => "profile", :id => @employee.id
  ...
  def change_reporting_manager
    ...
    @reporting_manager = HR::Employee.find(@emp.reporting_manager_id).first_name unless @emp.reporting_manager_id.nil?
    if request.post?
      @employee = HR::Employee.find(params[:id])
      HR::Employee.update(@employee, :reporting_manager_id => params[:employee][:reporting_manager_id])
  ...
  def update_reporting_manager_name
    employee = HR::Employee.find(params[:employee_reporting_manager_id])
    render :text => employee.first_name + ' ' + employee.last_name
  end

  def search
    @departments = HR::EmployeeDepartment.all
    @categories  = HR::EmployeeCategory.all
    @positions   = HR::EmployeePosition.all
    @grades      = HR::EmployeeGrade.all
  end
  ...
  def profile
    ...
    @reporting_manager = HR::Employee.find(@employee.reporting_manager_id).full_name unless @employee.reporting_manager_id.nil?
  ...
  def profile_payroll_details
    ...
    @employee = HR::Employee.find(params[:id])
    @payroll_details = HR::EmployeeSalaryStructure.find_all_by_employee_id(@employee, :order=>"payroll_category_id ASC")
  ...
  def view_all
    @departments = HR::EmployeeDepartment.active
  end

  def employees_list
    department_id = params[:department_id]
    @employees = HR::Employee.find_all_by_employee_department_id(department_id,:order=>'first_name ASC')
  ...
  def add_payslip_category
    @employee = HR::Employee.find(params[:emp_id])
  ...
  def create_monthly_payslip
    ...
    if request.post?
      ...
      unless salary_date.to_date < @employee.joining_date.to_date
        ...
        payslip_exists = HR::MonthlyPayslip.find_all_by_employee_id(@employee.id,
          :conditions => ["salary_date >= ? and salary_date < ?", start_date, end_date])
        if payslip_exists == []
          params[:manage_payroll].each_pair do |k, v|
            row_id = HR::EmployeeSalaryStructure.find_by_employee_id_and_payroll_category_id(@employee, k)
            category_name = PayrollCategory.find(k).name
            unless row_id.nil?
              HR::MonthlyPayslip.create(:salary_date=>start_date,:employee_id => params[:id],
  ...
  def view_payslip
    @employee = HR::Employee.find(params[:id])
    @salary_dates = HR::MonthlyPayslip.find_all_by_employee_id(params[:id], :conditions=>"is_approved = true",:select => "distinct salary_date")
  ...
  def update_monthly_payslip
    ...
    @monthly_payslips = HR::MonthlyPayslip.find_all_by_salary_date(params[:salary_date],
  ...
  def delete_payslip
    ...
    @monthly_payslip = HR::MonthlyPayslip.find_all_by_employee_id_and_salary_date(params[:id], params[:id2])
  ...
  def view_attendance
    @employee = HR::Employee.find(params[:id])
    @attendance_report = HR::EmployeeAttendance.find_all_by_employee_id(@employee.id)
    @leave_types = HR::EmployeeLeaveType.find(:all, :conditions => "status = true")
    @leave_count = HR::EmployeeLeave.find_all_by_employee_id(@employee,:joins=>:employee_leave_type,:conditions=>"status = true")
    @total_leaves = 0
    @leave_types.each do |lt|
      leave_count = HR::EmployeeAttendance.find_all_by_employee_id_and_employee_leave_type_id(@employee.id,lt.id).size
  ...
  def employee_leave_count_edit
    @leave_count = HR::EmployeeLeave.find_by_id(params[:id])
    @leave_type = HR::EmployeeLeaveType.find_by_id(params[:leave_type])
  ...
  def employee_leave_count_update
    ...
    leave = HR::EmployeeLeave.find_by_id(params[:id])
    leave.update_attributes(:leave_count => available_leave.to_f)
    @employee = HR::Employee.find(leave.employee_id)
    @attendance_report = HR::EmployeeAttendance.find_all_by_employee_id(@employee.id)
    @leave_types = HR::EmployeeLeaveType.find(:all, :conditions => "status = true")
    @leave_count = HR::EmployeeLeave.find_all_by_employee_id(@employee)
    @total_leaves = 0
    @leave_types.each do |lt|
      leave_count = HR::EmployeeAttendance.find_all_by_employee_id_and_employee_leave_type_id(@employee.id,lt.id).size
      @total_leaves = @total_leaves + leave_count
  ...
  def select_department
    ...
    @assigned_employee = HR::EmployeesSubject.find_all_by_subject_id(@subject.id)
    @departments = HR::EmployeeDepartment.find(:all, :conditions =>{:status=>true})
    render :update do |page|
  ...
  def update_employees
    @subject = Subject.find(params[:subject_id])
    @employees = HR::Employee.find_all_by_employee_department_id(params[:department_id], :conditions =>{:status=>true})
    render :update do |page|
  ...
  def assign_employee
    ...
    @employees = HR::Employee.find_all_by_employee_department_id(Employee.find(params[:id]).employee_department_id)
    HR::EmployeesSubject.create(:employee_id => params[:id], :subject_id => params[:id1])
    @assigned_employee = HR::EmployeesSubject.find_all_by_subject_id(@subject.id)
  ...
  def remove_employee
    ...
    if TimetableEntry.find_all_by_subject_id_and_employee_id(@subject.id,params[:id]).blank?
      HR::EmployeesSubject.find_by_employee_id_and_subject_id(params[:id], params[:id1]).destroy
    else
  ...
  def update_rejected_employee_list
    department_id = params[:department_id]
    #@employees = HR::Employee.find_all_by_employee_department_id(department_id)
    @employees = HR::MonthlyPayslip.find(:all, :conditions =>"is_rejected is true", :group=>'employee_id', :joins=>"INNER JOIN employees on monthly_payslips.employee_id = employees.id")
  ...
  def edit_rejected_payslip
    ...
    @employee = HR::Employee.find(params[:id])
    @monthly_payslips = HR::MonthlyPayslip.find_all_by_salary_date(@salary_date,
  ...
  def view_rejected_payslip
    ...
    @payslips = HR::MonthlyPayslip.find_all_by_employee_id(params[:id], :conditions =>"is_rejected is true", :group=>'salary_date')
    @emp = HR::Employee.find(params[:id])
  end
  ...
  def one_click_payslip_generation
    ...
    start_date = salary_date - ((salary_date.day - 1).days)
    end_date = start_date + 1.month
    employees = HR::Employee.find(:all)
    unless(finance_manager.nil? and finance.nil?)
      ...
      Delayed::Job.enqueue(DelayedReminderJob.new( :sender_id  => @user.id,
          ...
          :body=>body ))
      employees.each do|e|
        payslip_exists = HR::MonthlyPayslip.find_all_by_employee_id(e.id,
          :conditions => ["salary_date >= ? and salary_date < ?", start_date, end_date])
        if payslip_exists == []
          salary_structure = HR::EmployeeSalaryStructure.find_all_by_employee_id(e.id)
          unless salary_structure == []
            salary_structure.each do |ss|
              HR::MonthlyPayslip.create(:salary_date=>start_date,
                :employee_id=>e.id,
  ...
  def payslip_revert_date_select
    @salary_dates = HR::MonthlyPayslip.find(:all, :select => "distinct salary_date")
  ...
  def one_click_payslip_revert
    unless params[:one_click_payslip][:salary_date] == ""
      ...
      employees = HR::Employee.find(:all)
      employees.each do|e|
        payslip_record = HR::MonthlyPayslip.find_all_by_employee_id(e.id,
          :conditions => ["salary_date >= ? and salary_date < ?", start_date, end_date])
  ...
  def leave_management
    user = current_user
    @employee = user.employee_record
    @all_employee = HR::Employee.find(:all)
    @reporting_employees = HR::Employee.find_all_by_reporting_manager_id(@employee.id)
    @leave_types = HR::EmployeeLeaveType.find(:all)
    @total_leave_count = 0
  ...
  def all_employee_leave_applications
    ...
    @employee = HR::Employee.find(params[:id])
    @departments = HR::EmployeeDepartment.find(:all, :order=>"name ASC")
    @employees = []
  ...
  def department_payslip
    @departments = HR::EmployeeDepartment.find(:all, :conditions=>"status = true", :order=> "name ASC")
    @salary_dates = HR::MonthlyPayslip.find(:all,:select => "distinct salary_date")
    if request.post?
      post_data = params[:payslip]
      unless post_data.blank?
        if post_data[:salary_date].present? and post_data[:department_id].present?
          @payslips = HR::MonthlyPayslip.find_and_filter_by_department(post_data[:salary_date],post_data[:department_id])
        else
  ...
  def view_employee_payslip
    @monthly_payslips = HR::MonthlyPayslip.find(:all,:conditions=>["employee_id=? AND salary_date = ?",params[:id],params[:salary_date]],:include=>:payroll_category)
    ...
    @salary  = HR::Employee.calculate_salary(@monthly_payslips, @individual_payslips)
  end

  #PDF methods

  def view_employee_payslip_pdf
    @employee = HR::Employee.find(:first,:conditions => {:id => params[:id]})
  ...
  def department_payslip_pdf
    @department = HR::EmployeeDepartment.find(params[:department])
    @employees = HR::Employee.find_all_by_employee_department_id(@department.id)
  ...
  def individual_payslip_pdf
    @employee = HR::Employee.find(params[:id])
    @department = HR::EmployeeDepartment.find(@employee.employee_department_id).name
    @currency_type = CustomSetting.find_by_config_key("CurrencyType").config_value
    @category = HR::EmployeeCategory.find(@employee.employee_category_id).name
    @grade = HR::EmployeeGrade.find(@employee.employee_grade_id).name unless @employee.employee_grade_id.nil?
    @position = HR::EmployeePosition.find(@employee.employee_position_id).name
  ...
  def employee_individual_payslip_pdf
    @employee = HR::Employee.find(:first,:conditions=>"id=#{params[:id]}")
    if @employee.blank?
      @employee = HR::ArchivedEmployee.find(:first,:conditions=>"former_id=#{params[:id]}")
  ...
  def remove
    @employee = HR::Employee.find(params[:id])
    associate_employee = HR::Employee.find(:all, :conditions=>["reporting_manager_id=#{@employee.id}"])
  ...
  def remove_subordinate_employee
    @current_manager = HR::Employee.find(params[:id])
    @associate_employee = HR::Employee.find(:all, :conditions=>["reporting_manager_id=#{@current_manager.id}"])
    @departments = HR::EmployeeDepartment.find(:all)
    @categories  = HR::EmployeeCategory.find(:all)
    @positions   = HR::EmployeePosition.find(:all)
    @grades      = HR::EmployeeGrade.find(:all)
    if request.post?
      @associate_employee.each do |e|
        HR::Employee.update(e, :reporting_manager_id => params[:employee][:reporting_manager_id])
      end
  ...
  def change_to_former
    ...
    if request.post?
      ...
      HR::EmployeesSubject.destroy_all(:employee_id=>@employee.id)
  ...
  def advanced_search_pdf
    ...
    if params[:status] == 'true'
      @employee_ids.each do |s|
        employee = HR::Employee.find(s)
        @employees1.push employee
      end
    elsif params[:status] == 'false'
      @employee_ids.each do |s|
        employee = HR::ArchivedEmployee.find(s)
        @employees1.push employee
      end
  ...
  def one_click_approve
    @dates = HR::MonthlyPayslip.find_all_by_salary_date(params[:salary_date],:conditions => ["is_approved = false"])
    @salary_date = params[:salary_date]
    render :update do |page|
  ...
  def one_click_approve_submit
    dates = HR::MonthlyPayslip.find_all_by_salary_date(Date.parse(params[:date]))
