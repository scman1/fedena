...
class CustomSettingsController < ApplicationController
  ...
  def settings
    @config = CustomSetting.get_multiple_configs_as_hash ['InstitutionName', 'InstitutionAddress', 'InstitutionPhoneNo', \
    ...
    @enabled_grading_types = CustomSetting.get_grading_types
    @time_zones = TimeZone.all
    @school_detail = SchoolDetail.first || SchoolDetail.new
    @countries=Country.all
    if request.post?
      CustomSetting.set_config_values(params[:custom_setting])
      ...
      CustomSetting.clear_school_cache(@current_user)
