...
class FedenaPlugin
  ...
  def self.register=(plugin_details)
    unless AVAILABLE_MODULES.collect{|mod| mod[:name]}.include?(plugin_details[:name])
      ...
      Authorization::AUTH_DSL_FILES << "#{File.expand_path(Rails.root)}/vendor/plugins/#{plugin_details[:name]}/#{plugin_details[:auth_file]}" unless plugin_details[:auth_file].blank?
