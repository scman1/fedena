...
class CustomSettingTest <  ActiveSupport::TestCase


  context 'custom_setting' do
    ...
    CONFIG_HASH.each do |key,value|
      should "be able to get config value for #{key}" do
        config_value = CustomSetting.get_config_value(key)
    ...
    should "not be able to set config value for NetworkType than Online,Offline" do
      config = CustomSetting.find_by_config_key("NetworkState")
      config.config_value = "Test"
      assert !config.valid?
    end
